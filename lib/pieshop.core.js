...
(function (global) {
    ...
    ResourceMeta.prototype.getForwardRelationNames = function() {
        if(this._cachedForwardRelationNames) {
            return this._cachedForwardRelationNames;
        }
        this._cachedForwardRelationNames = [];
        for(var i in this.fields) {
            if(this.fields[i].rel) {
                this._cachedForwardRelationNames.push(this.fields[i].name);
            }
        }
        return this._cachedForwardRelationNames;
    };
    ...
    var resource_factory = function(opts) {
        var fields = global.require('pieshop.fields'),
            ...
            ResourceProto = {
                '_getValueDict':function() {
                    var output = {};
                    for(var field_name in this._meta.fields) {
                        if(!this._meta.get_field_by_name(field_name).primary_key) {
                ...
                'save':function(callback) {
                    ...
                    var pk_name = this._meta.get_field_by_name('pk').name,
                        pk_value = this[pk_name]; 
                    if(pk_value !== undefined) {
                        Resource.objects.filter({'pk':pk_value}).update(this._getValueDict(), function(objects, err) {
                            if(!err && objects.length > 0) { objects = objects[0]; }
                            callback(objects, err);
                        });
                    } else {
                        Resource.objects.create(this._getValueDict(), function(objects, err) {
                            if(!err && objects.length > 0) { objects = objects[0]; }
                            callback(objects, err);
                        });
                    }
                ...
                'erase':function(callback) {
                    callback = callback || function() {};
                    var pk_name = this._meta.get_field_by_name('pk').name; 
                    return Resource.objects.filter({'pk':this[pk_name]}).erase(callback);
                }
            },
            model_fields = {},
            metaclass_opts = {},
